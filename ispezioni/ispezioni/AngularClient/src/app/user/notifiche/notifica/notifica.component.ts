...
export class NotificaComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.us.user.subscribe(user => this.role = user.ruoloUtente);
  ...
  caricaCantiere(nuovoStato: any) {
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      this.ns.getNotificaInfo(params['idNotifica']).subscribe((n) => {
        console.log(n);
  ...
  onSubmit(e: any): void {
    ...
    if(statoSelezionato.stato.toLocaleUpperCase().includes("BOZZ") || statoSelezionato.stato.toLocaleUpperCase().includes("ELIMIN") || statoSelezionato.stato.toLocaleUpperCase().includes("INTEGR")){  //se bozza o elimin non faccio check db
      ...
      this.ns.checkNotifica(this.noticeForm.value).subscribe((res:any) => { //check db
        if(!res.esito)
  ...
  salvaStato(nuovoStato: any) {
    ...
    console.log(this.noticeForm.value);
    this.ns.updateNotifica(this.noticeForm.value).subscribe((res: any) => {
      ...
      if (res.esito) {
        if (res.msg == 'NUOVO ID') //{
          ...
          this.router.navigate(['notifiche/notifica'], {
            queryParams: {
  ...
  download(nuovoPdf: boolean) {
    ...
    this.ns.downloadNotifica(dataToSend, nuovoPdf, 'notificaPreliminare').subscribe(
      (data) => {
        Utils.download(
          data,
    ...
    if(this.role != 'ASL - Notifiche Preliminari'){
      this.ns.downloadNotifica(dataToSend, nuovoPdf, 'cartelloNotifica').subscribe(
        (data) => {
          Utils.download(
            data,
  ...
  checkForm(statoSelezionato:string): boolean{
    ...
    var patternNumericoPositivoDecimale = '^[1-9][0-9]*(\.[0-9]{0,2})?$';
    ...
    if(!statoSelezionato.toLocaleUpperCase().includes("BOZZ")){ //se non è una bozza controllo che ci sia il cf
      ...
      cantiere.ammontare = cantiere.ammontare.toString().replace(',', '.');
      if(!cantiere.ammontare.toString().match(patternNumericoPositivoDecimale)){
        Swal.fire({
          text: 'Il valore ammontare complessivo presunto dei lavori (in €) deve essere numerico, maggiore di 0. Sono consentite al massimo due cifre decimali',
          icon: 'warning',
        });
        return false;
      }else{
        this.noticeForm.get('cantiere')?.patchValue({ammontare: cantiere.ammontare})
      }
