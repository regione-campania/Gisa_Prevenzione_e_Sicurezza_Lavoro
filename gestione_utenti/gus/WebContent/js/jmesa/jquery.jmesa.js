(function ($) {
    ...
    var coreapi = {
        ...
        addFilterDateToLimit : function(id, property, valueIni,valueFin) {
            /*First remove the filter if it is set on the limit,
              and then set the page back to the first one.*/
            this.removeFilterFromLimit(id, property);
            this.setPageToLimit(id, '1');
            ...
            var limit = this.getTableFacade(id).limit;
            var filter = new classes.Filter(property, valueIni);
            limit.addFilter(filter);
            filter = new classes.Filter(property, valueFin);
            limit.addFilter(filter);
        },
    ...
    var filterapi = {
        ...
        createDynDateFilter : function(filter, id, property) {
            ...
            //cell.html('<div id="dynFilterDateDiv"><input readonly=\"readonly\"id="dynFilterDateInput"name="filter" style="width:' + (width + 2) + 'px" value="" /></div>');

            
            var inputIni = $('#' + property + 'Iniziale');
            var inputFin = $('#' + property + 'Finale');
...
//             });
            ...
            var originalValueIni = inputIni.val();
            var originalValueFin = inputFin.val();
            var widthIni = inputIni.width();
            var widthFin = inputFin.width();
            
            inputIni.val(originalValueIni);
            inputFin.val(originalValueFin);
            inputIni.focus();
...
//            });
            ...
            //$(input).change(function() {
                var changedValueIni = inputIni.val();
                var changedValueFin = inputFin.val();
                //input.val('');
                inputIni.css('overflow', 'hidden');
                inputFin.css('overflow', 'hidden');
                inputIni.val=changedValueIni;
                inputFin.val=changedValueFin;
                
                if(changedValueIni=="")
                	changedValueIni="nullo";
                if(changedValueFin=="")
                   	changedValueFin="nullo";
                $.jmesa.addFilterDateToLimit(dynDateFilter.id, dynDateFilter.property, changedValueIni+"-----"+changedValueFin);
                $.jmesa.onInvokeAction(dynDateFilter.id, 'filter');
                dynDateFilter = null;
            //});		
        },
