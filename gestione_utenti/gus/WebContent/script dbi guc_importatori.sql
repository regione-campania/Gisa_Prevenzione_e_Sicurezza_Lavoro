...
BEGIN
	...
	IF (us_id is null) THEN	
		...
		INSERT INTO access ( user_id, username, password, contact_id, site_id, role_id, enteredby, modifiedby, timezone, currency, language, enabled, expires,id_importatore ) 
		VALUES (  us_id, usr, password, con_id, site_id, role_id, 964, 964, 'Europe/Berlin', 'EUR', 'it_IT', enabled, expires::timestamp without time zone,id_importatore); 
...
BEGIN
	...
	mod = now();
	UPDATE access set password=password_new, username = username_new, site_id = site_id_new, role_id = role_id_new, modified = mod, modifiedby = 964, enabled = enabled_new, expires = expires_new::timestamp without time zone, id_importatore = id_importatore_new
	...
	WHERE contact_id in ( select a.contact_id from access a where a.username = username_new and a.trashed_date is null);
...
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
ALTER FUNCTION dbi_update_utente(character varying, character varying, integer, integer, boolean, timestamp with time zone, character varying, character varying, character varying, character varying, text, text, character varying, character varying, integer)
  OWNER TO postgres;
...
CREATE OR REPLACE FUNCTION dbi_enable_utente(usr character varying, importatore_id integer)
  RETURNS text AS
$BODY$
   DECLARE
msg text ;
...
BEGIN
	us_id :=  (select user_id from access a where a.username = usr and a.trashed_date is null);
	IF (us_id is not null) THEN
		UPDATE access_collegamento set enabled = true where id_utente=us_id and id_collegato=importatore_id;
	END IF;
	msg = 'OK';
	RETURN msg;
END
...
CREATE OR REPLACE FUNCTION dbi_check_esistenza_utente_by_struttura (usr character varying, importatore_id integer)
  RETURNS text AS
$BODY$
   DECLARE
msg text ;
...
BEGIN
	us_id :=  (select user_id from access a where a.username = usr and a.trashed_date is null);
	tot := (select count(*) from access_collegamento u where u.id_utente=us_id and id_collegato=importatore_id);
	IF (tot=0) THEN
		msg='KO';
	ELSE 	
		msg='OK';
...
CREATE OR REPLACE FUNCTION dbi_get_lista_importatori()
  RETURNS TABLE(id_importatore integer, ragione_sociale text, partita_iva text, id_asl integer) AS
$BODY$
DECLARE
r RECORD;	
BEGIN
FOR id_importatore, ragione_sociale, partita_iva
in
select id, ragione_sociale , partita_iva, id_asl from opu_operatore where trashed_date is null
LOOP
        RETURN NEXT;
END LOOP;
     RETURN;
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION dbi_get_lista_importatori()
  OWNER TO postgres;
