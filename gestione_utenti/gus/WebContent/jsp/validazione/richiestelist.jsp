...
<div align="center">
<% if (limit > 0){ %>
Sono mostrate solo le ultime <%=limit %> richieste inserite. Per visualizzarle tutte, cliccare su 
<a href="validazione.ToValidazione.us?limit=-2" onClick="loadModalWindow();" style="margin: 0px 0px 0px 50px; font-size: 20px">Mostra tutte</a>
<%} %>
</div>
...
<input type="button" onClick="mostraProcessa('<%=ric.getNumeroRichiesta()%>', '<%=ric.getIdTipoRichiesta() %>', '<%=ric.getIdTipologiaUtente() %>', '<%=(ric.getIdRuoloGisa()>0 || ric.getIdRuoloGisaExt()>0) ? "true" : "false" %>');" value="ACCETTA"/>
