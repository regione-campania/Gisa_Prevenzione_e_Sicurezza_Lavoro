...
<!-- MODIFICA PERMESSI | ELIMINA ELEMENTO | SYNC -->
...
<table style="width: 400px;" border="0" align="center">
	<tbody>
		...
		<tr>
			<td align="center"><input type="text" name="role"
				value="<%=RoleBean.getRole()%>"></td>
			<td align="center"><input type="text" name="descr"
	...
	<tbody>
		<tr>
			<td align="center"><font color="#0000FF" size="3"><b>TRASH</b></font><input
				type="checkbox" class='TR' name="all" onClick="toggle(this)" value="all"></input></td>
		...
		<%
			...
			int riga = 0;
			for (j = 0; j < RoleBean.getPerm().size(); j++) {
		%>
		<tr bgcolor=<%=colore%> onmouseover="this.style.color = 'red';" onmouseout="this.style.color = 'black';">
			<td align="center"><input type="checkbox" class='TR' name="trash<%=j%>"
				value="trash<%=j%>"></input></td>
			<td><%=RoleBean.getPerm().get(j).split(";")[0]%></td>
			<td align="center"><input type="radio" name="perm<%=j%>"
				value="r"
				<%if (RoleBean.getPerm().get(j).split(";")[2].equals("r")) {%>
				checked="checked" <%}%>></input></td>
			<td align="center"><input type="radio" name="perm<%=j%>"
				value="w"
				<%if (RoleBean.getPerm().get(j).split(";")[2].equals("w")) {%>
				checked="checked" <%}%>></input></td>
		</tr>
		<%
			if (riga == 0) {
