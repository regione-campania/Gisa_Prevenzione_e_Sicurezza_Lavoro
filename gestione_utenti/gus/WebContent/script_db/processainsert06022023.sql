-- modifica per rendere bloccante ok-ko per gisa_Ext
...
BEGIN
	...
	ep_guc := 'KO;;KO'; -- decommento quanto commentato per flusso 315
	...
	if (length(check_gisa)=0  and length(check_bdu) = 0 and length(check_vam) = 0 --and length(check_digemon) = 0 IGNORO I KO SU DIGEMON E VADO AVANTI COMUNQUE
									              and length(check_gisa_ext) = 0 -- non GNORO I KO SU gisa ext
