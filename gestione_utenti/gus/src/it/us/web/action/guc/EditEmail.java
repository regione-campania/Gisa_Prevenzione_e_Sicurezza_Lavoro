...
public class EditEmail extends GenericAction
{
		...
		try{
			...
			if( utentiL1.size() > 0 ){
				...
				BeanUtils.populate(u, req.getParameterMap());
					...
					u.setEmail(stringaFromRequest("email") );
					...
					String urlReloadUtenti = "";
					  ...
					  for(int i = 0; i <listaEndPointConnectorOperazione.size(); i++)
					  {
						...
						epcModificaUtente = listaEndPointConnector.getByIdOperazioneIdEndPoint(Operazione.MODIFICAEMAIL, epc.getEndPoint().getId()); 

						if (esisteUtente) {
							
							okEndpoint = update_email_Utente(u, epcModificaUtente, conn);
						...
						System.out.println("Aggiornata email su " + (epcModificaUtente.getEndPoint().getNome()));
						...
						if(!okEndpoint){
							if (epc.getEndPoint().getId()!= EndPoint.GUC){
							logger.error("Errore durante la modifica email dell'utente " + u.getUsername() + " nell'endpoint " + epcModificaUtente.getEndPoint().getNome());
							...
							if (urlReloadUtenti!=null && !urlReloadUtenti.equals("")){ 
								...
								logReload.setTipo_op("edit_email");
								logReload.insert(db);
	...
	private Boolean update_email_Utente(Utente u, EndPointConnector epc, Connection conn) throws Exception{
		Boolean okEndpoint = false;
		okEndpoint = this.gestioneAggiornamentoEmailUtente(epc, u, conn);
		return okEndpoint;
	}
